...
- **ingest navs** — self-healing incremental: bulk-fetches `MAX(nav_date)` → today via AMFI's history endpoint (inclusive watermark, so a partial last day is re-fetched and repaired), then layers today's NAVAll snapshot on top. ~10 s–6 min on a warm DB depending on the gap; raises `IngestError` on an empty `nav_daily` (run `--backfill` first); `--backfill` does full history (5–15 min).
...
- **`CoverageError` (Gate A halt)** — a BLOCKING source (NAV/benchmark/risk-free/scheme_master) is empty, stale, too-short-history, below the 50% entity floor, or — for NAV only — has an **interior gap** (`[INTERIOR_GAP]`: more than `max_nav_interior_gap_days` trading days in the last 30 with a sub-floor NAV row count, i.e. a hole *between* have-from and have-till that end-recency checks can't see). The rendered Gate A table names the source, the gap, and the remediation — for `[INTERIOR_GAP]` that is `mfs ingest navs --since <first missing date>` — run it and re-try. Do NOT bypass; do not compute on incomplete base data. (A partial gap *above* the floor shows as `[GAP]` but passes — surface it, don't halt.)
