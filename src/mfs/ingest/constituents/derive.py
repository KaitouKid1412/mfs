...
``holdings_monthly`` (Phase 3.C). Circularity note (audit dim6): the
constituent "source" is itself a fund portfolio — the tracker fund is
auto-selected and recorded in the run summary so the provenance is always
attributable.

D9: this module is the pipeline-integrated home of what used to live only in
``tools/derive_constituents.py`` (now a thin wrapper). The pipeline runs
``derive_latest_month`` as the stage 'derive constituents (latest month)'
right after holdings ingest, so ``index_constituents_monthly`` accrues a new
month every cycle instead of being a one-off manual artifact; the
``max_constituents_lag_days: 75`` freshness gate halts the run if this stage
silently stalls.

For each target benchmark ticker it AUTO-SELECTS the best-ingested tracker
fund by name pattern + an index-size sanity check (rejecting broad-index or
factor-variant funds that happen to match the name), so the derivation is
robust to which AMC adapters have run. Indices whose tracker isn't ingested
yet are reported SKIPPED rather than guessed.
...
The ingest step (`mfs ingest constituents` / the pipeline stage right after
this one) upserts those CSVs into ``index_constituents_monthly``.
"""
...
# NOTE: indices that DO have a Direct+Growth index-fund tracker in
# holdings_monthly (e.g. NIFTY 50 / NIFTY Bank — verified live 2026-06-13)
# belong in _SPECS below, which is DB-only and needs no network.
...
_SPECS: dict[str, dict] = {
    ...
    # D9 [REVIEW-EXTENDED]: NIFTY 50 TRI is a first-class derived ticker (it
    # was previously only the hidden "_nifty_50" hybrid-sleeve helper). Both
    # it and NIFTY Bank TRI have ingested Direct+Growth index-fund trackers
    # (verified live 2026-06-13: ~20 Nifty 50 trackers at 50 equity rows;
    # 8 Nifty Bank trackers — Bandhan/ICICI/Navi/DSP/SBI/Nippon/Invesco/Axis —
    # at 14 equity rows), so they derive from the DB, NOT via _ETF_TRACKERS.
    "nifty_50_tri": {"primary": "%nifty 50%", "excludes": _FACTOR_EXCLUDES + ["%next 50%", "%500%", "%100%", "%200%", "%midcap%", "%smallcap%", "%bank%"], "expected": 50,
                     "alt_primary": "%nifty index%"},
    "nifty_bank_tri": {"primary": "%nifty bank%", "excludes": _FACTOR_EXCLUDES + ["%psu bank%", "%private bank%", "%ex-bank%", "%ex bank%"], "expected": 12},
    # --- sectoral / thematic ---
    ...
    "nifty_50_hybrid_50_50_tri": {"sleeve_of": "nifty_50_tri", "expected": 50},
    "nifty_50_hybrid_65_35_tri": {"sleeve_of": "nifty_50_tri", "expected": 50},
    "nifty_equity_savings_tri": {"sleeve_of": "nifty_50_tri", "expected": 50},
}

# Hybrid sleeves of the NIFTY 50 get the explanatory note in the summary.
_NIFTY50_SLEEVE = "nifty_50_tri"
...
def latest_holdings_ym() -> str:
    """Most recent data month in holdings_monthly as 'YYYY-MM' — the month
    the pipeline stage derives. Raises IngestError when no holdings are
    ingested (the derive stage is required; an empty holdings table at this
    point in the run is systemic)."""
    from mfs.db import queries as q

    latest = q.latest_holdings_date()
    if latest is None:
        raise IngestError(
            "derive constituents: holdings_monthly is empty — run "
            "`mfs ingest holdings` first."
        )
    return f"{latest.year:04d}-{latest.month:02d}"


def derive_month(ym: str, *, dry_run: bool = False) -> dict:
    """Derive constituent CSVs for every spec'd benchmark for data month
    ``ym`` ('YYYY-MM'). Returns a summary dict:
    ``{"ym", "n_ok", "n_total", "rows": [(slug, status, n, note), ...]}``.

    Per-index failures are SKIPPED rows, never exceptions — one missing
    tracker must not stop the others (mirrors per-AMC isolation). Systemic
    failures (DB down) do raise and halt the pipeline stage.
    """
    ym_first = f"{ym}-01"
    ...
    for slug in derived:
        ...
        if spec.get("sleeve_of"):
            ...
            note = ("NIFTY 50 equity sleeve (debt sleeve excluded by design)"
                    if spec["sleeve_of"] == _NIFTY50_SLEEVE
    ...
    n_ok = 0
    for slug, status, n, _note in sorted(summary):
    ...
    log.info(
        "constituents.derive_month_done",
        ym=ym, n_ok=n_ok, n_total=len(_SPECS), dry_run=dry_run,
        skipped=[s for s, st, _n, _ in summary if st == "SKIPPED"],
    )
    return {"ym": ym, "n_ok": n_ok, "n_total": len(_SPECS), "rows": sorted(summary)}


def derive_latest_month(*, dry_run: bool = False) -> dict:
    """Pipeline-stage entry point: derive constituents for the most recent
    holdings data month (D9). The downstream 'ingest constituents' stage then
    upserts every manual CSV — including the ones written here — into
    ``index_constituents_monthly``."""
    return derive_month(latest_holdings_ym(), dry_run=dry_run)


def render_summary(result: dict) -> str:
    """Human-readable table for the tool wrapper / operator runs."""
    lines = [
        f"\n{'benchmark slug':36s} {'status':7s} {'n':>4s}  note",
        "-" * 100,
    ]
    for slug, status, n, note in result["rows"]:
        lines.append(f"{slug:36s} {status:7s} {n:>4d}  {note}")
    lines.append(
        f"\nResolved {result['n_ok']}/{result['n_total']} benchmark tickers "
        f"for {result['ym']}."
    )
    lines.append(
        "OK* = hybrid benchmark modeled by its NIFTY 50 equity sleeve "
